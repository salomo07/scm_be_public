...
func AddAccessMenu(ctx *fasthttp.RequestCtx, user models.User) {
	...
	if err != "" {
		...
		res, err, _ := services.FindOne(user.IdCompany, consts.Coll_AccessMenu, `{"idrole":"`+accessModel.IdRole+`","idmenu":"`+accessModel.Idmenu+`"}`, "", "")
		if err != "" {
...
func UpdateAccessMenu(ctx *fasthttp.RequestCtx, user models.User) {
	...
	if err != "" {
		...
		queryUpdateAccessMenu := `{"idrole":"` + accessModel.IdRole + `","idmenu":"` + accessModel.Idmenu + `"}`
